const pt = {
  title: "Publicações <highlight>Acadêmicas</highlight>",
  seeMoreText: "Ver mais publicações",
  readArticle: "Ler artigo",
  articles: [
    {
      title: "Gifflar: A framework to generate smart contracts on the fly",
      link: "https://dl.acm.org/doi/abs/10.5555/3507788.3507820",
      items: [
        { title: "Local", description: "Toronto, Canada" },
        { title: "Ano", description: "2021" },
        {
          title: "Em",
          description:
            "31st Annual International Conference on Computer Science and Software",
        },
      ],
    },
    {
      title:
        "Blockchain applied to academic environments as a way to ensure educational process quality control",
      link: "https://ieeexplore.ieee.org/document/9149491",
      items: [
        { title: "Local", description: "Bogota, Cundinamarca, Colombia" },
        { title: "Ano", description: "2020" },
        {
          title: "Em",
          description:
            "IEEE World Conference on Engineering Education (EDUNINE)",
        },
      ],
    },
    {
      title: "IoTCocoa: an IoT platform to assist gourmet cocoa production.",
      link: "https://ieeexplore.ieee.org/abstract/document/8937903",
      items: [
        { title: "Local", description: "Salvador, Brasil" },
        { title: "Ano", description: "2019" },
        {
          title: "Em",
          description:
            "IEEE Latin-American Conference on Communications (LATINCOM)",
        },
      ],
    },
    {
      title: "I2oTology: Tracking-Oriented Ontology.",
      link: "https://ceur-ws.org/Vol-2228/short5.pdf",
      items: [
        { title: "Local", description: "São paulo, Brasil" },
        { title: "Ano", description: "2018" },
        {
          title: "Em",
          description: "ONTOBRAS",
        },
      ],
    },
  ],
};

const en = {
  title: "<highlight>Academic</highlight> Publications",
  seeMoreText: "See more publications",
  readArticle: "Read article",
  articles: [
    {
      title: "Gifflar: A framework to generate smart contracts on the fly",
      link: "https://dl.acm.org/doi/abs/10.5555/3507788.3507820",
      items: [
        { title: "Place", description: "Toronto, Canada" },
        { title: "Year", description: "2021" },
        {
          title: "In",
          description:
            "31st Annual International Conference on Computer Science and Software",
        },
      ],
    },
    {
      title:
        "Blockchain applied to academic environments as a way to ensure educational process quality control",
      link: "https://ieeexplore.ieee.org/document/9149491",
      items: [
        { title: "Place", description: "Bogota, Cundinamarca, Colombia" },
        { title: "Year", description: "2020" },
        {
          title: "In",
          description:
            "IEEE World Conference on Engineering Education (EDUNINE)",
        },
      ],
    },
    {
      title: "IoTCocoa: an IoT platform to assist gourmet cocoa production.",
      link: "https://ieeexplore.ieee.org/abstract/document/8937903",
      items: [
        { title: "Place", description: "Salvador, Brasil" },
        { title: "Year", description: "2019" },
        {
          title: "In",
          description:
            "IEEE Latin-American Conference on Communications (LATINCOM)",
        },
      ],
    },
    {
      title: "I2oTology: Tracking-Oriented Ontology.",
      link: "https://ceur-ws.org/Vol-2228/short5.pdf",
      items: [
        { title: "Place", description: "São paulo, Brasil" },
        { title: "Year", description: "2018" },
        {
          title: "In",
          description: "ONTOBRAS",
        },
      ],
    },
  ],
};

export const content = { pt, en };
